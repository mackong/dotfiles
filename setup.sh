...
if [ "$(uname)" == "Darwin" ]; then
    brew install antlr ascii autoconf automake bat bison chezscheme cmake csvlens coreutils dos2unix extra-cmake-modules ffmpeg flex git-lfs go graphviz jq maven maxima meson moreutils neovim node pre-commit python-lsp-server radare2 rcm ripgrep texlive tmux tree tree-sitter uv websocat xz

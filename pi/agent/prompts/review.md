...
Review target: GitHub pull request `$1`.

Gather this target's diff with (instead of any local `git diff`):
1. `gh pr view $1 --json title,body,author,baseRefName,headRefName,state,additions,deletions,changedFiles,labels` for context
2. `gh pr diff $1` for the unified diff
